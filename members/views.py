...
class AdhesionsJsonView(LoginRequiredMixin, View):
    def serie(self, season, GET):
        ...
        if GET['function']:
            qs = qs.filter(nominations__function__category=GET['function'], nominations__main=True)
        if GET['rate']:
...
class TableauFunctionsView(TableauView):
    values = ('nominations__function__category', )
    title = "fonction"

    def row_key(self, obj):
        if obj['nominations__function__category'] is None:
            return "Autre"
        return dict(Function.CATEGORY_CHOICES)[obj['nominations__function__category']]
...
class YoungsHeadcountWidget(widget.Widget):
    ...
    def get_context_data(self):
        ...
        ref_date = date.replace(year=date.year - 1, day=28 if leap_year else date.day)
        qs = Adhesion.objects.filter(season=season, date__lte=date)
        qs = qs.filter(nominations__function__category=0, nominations__main=True)
        qs = qs.exclude(structure__number__in=SVN_NUMBERS + CPN_NUMBERS)
        qs = qs.aggregate(headcount=Count('id'))
        ref_qs = Adhesion.objects.filter(season=season - 1, date__lte=ref_date)
        ref_qs = ref_qs.filter(nominations__function__category=0, nominations__main=True)
...
class StagiaireHeadcountWidget(widget.Widget):
    ...
    def get_context_data(self):
        ...
        ref_date = date.replace(year=date.year - 1, day=28 if leap_year else date.day)
        qs = Adhesion.objects.filter(season=season, date__lte=date)
        qs = qs.filter(nominations__function__category=3, nominations__main=True)
        qs = qs.aggregate(headcount=Count('id'))
        ref_qs = Adhesion.objects.filter(season=season - 1, date__lte=ref_date)
        ref_qs = ref_qs.filter(nominations__function__category=3, nominations__main=True)
        ref_qs = ref_qs.aggregate(headcount=Count('id'))
        return {
